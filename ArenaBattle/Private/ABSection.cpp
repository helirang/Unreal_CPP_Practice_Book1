...
AABSection::AABSection()
{
	...
	for (FName GateSocket : GateSockets)
	{
		...
		UBoxComponent* NewGateTrigger = CreateAbstractDefaultSubobject<UBoxComponent>(*GateSocket.ToString().Append(TEXT("Trigger")));
...
void AABSection::BeginPlay()
{
	...
	SetState(bNoBattle ? ESetctionState::COMPLETE : ESetctionState::READY);
}

void AABSection::SetState(ESetctionState NewState)
{
	switch (NewState)
	{
	case AABSection::ESetctionState::READY:
	{
		Trigger->SetCollisionProfileName(TEXT("ABTrigger"));
	...
	case AABSection::ESetctionState::BATTLE:
	{
		...
		GetWorld()->GetTimerManager().
			SetTimer(SpawnNPCTimeHandle, FTimerDelegate::CreateUObject(this, &AABSection::OnNPCSpawn), EnemySpawnTime, false);

		GetWorld()->GetTimerManager().SetTimer(SpawnBoxTimeHandle, FTimerDelegate::CreateLambda([this]()->void {
	...
	case AABSection::ESetctionState::COMPLETE:
	{
		Trigger->SetCollisionProfileName(TEXT("NoCollision"));
		for (UBoxComponent* GateTrigger : GateTriggers)
		{
...
void AABSection::OnTriggerBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex,
	bool bFromSweep, const FHitResult &SweepResult)
{
	if (CurrentState == ESetctionState::READY)
	{
		SetState(ESetctionState::BATTLE);
	}
}

void AABSection::OnGateTriggerBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex,
	bool bFromSweep, const FHitResult &SweepResult)
{
...
{
	GetWorld()->GetTimerManager().ClearTimer(SpawnNPCTimeHandle);
